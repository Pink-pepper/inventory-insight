...
export async function getScenario(
  ...
  return {
    ...
    runs: (runs ?? []).map((r) => ({
      ...
      version: r.version,
      baselineSummary: r.baseline_summary as ScenarioRunSummaryRecord["baselineSummary"],
      scenarioSummary: r.scenario_summary as ScenarioRunSummaryRecord["scenarioSummary"],
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        baseline_summary: snapshot.result.baselineSummary,
        scenario_summary: snapshot.result.scenarioSummary,
        row_results: {
        ...
        input_provenance: snapshot.inputProvenance,
        created_by: userId,
      })
      .select("id, version")
      .maybeSingle();
